        ...
        return (
            <div className="min-h-screen bg-gray-50 py-10">
                <div className="mx-auto max-w-4xl rounded-lg border bg-white p-6">
                    ...
                    <Link to="/" className="mt-3 inline-block text-[#0066b3] hover:underline">
                        Quay về trang chủ
                    </Link>
                </div>
            </div>
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div className="mt-4 flex items-end gap-3">
                                <span className="text-3xl font-bold text-[#f37021]">{formatVnd(product.price)}</span>
                                {product.oldPrice && (
                                    <span className="text-gray-400 line-through">{formatVnd(product.oldPrice)}</span>
                                )}
                            </div>

                            <p className="mt-4 text-sm text-gray-600">
                                Đánh giá: <span className="font-semibold">{product.rating ?? 4.5}/5</span>
                            </p>
                            ...
                            >
                                <span className="font-semibold text-[#0066b3]">Trả góp chỉ từ {formatVnd(Math.ceil(product.price / 9))}/tháng qua MoMo</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="w-full max-w-xl rounded-xl bg-white p-5 shadow-2xl">
                        <div className="mb-4 flex items-center justify-between">
                            <h2 className="text-lg font-semibold text-[#0066b3]">Bảng tính trả góp MoMo (ước tính)</h2>
                            <button
                                ...
                                <tbody>
                                    {installmentPreview.map((row) => (
                                        <tr key={row.months} className="border-b">
                                            ...
                                            <td className="px-3 py-2 font-semibold text-[#f37021]">{formatVnd(row.monthly)}</td>
                        ...
                        <p className="mt-3 text-xs text-gray-500">
                            Lưu ý: Đây là bảng tính nhẩm frontend. Số tiền thực tế phụ thuộc cấu hình trả góp backend và trạng thái đơn.
                        </p>
                    </div>
                </div>
            )}
